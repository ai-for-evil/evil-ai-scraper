"""Entity deduplicator — merge candidate cases into unique entities.

Ported from Shiv-Evil-AI-Finder/src/deduper.py.
"""
...
def _normalize_name(name: str) -> str:
    return "".join(
        ch.lower() for ch in name if ch.isalnum() or ch.isspace()
    ).strip()
...
class EntityDeduper:
    """Merge candidate cases referring to the same entity."""

    def dedupe(
        self,
        cases: List[CandidateCase],
        seed_names: List[str] | None = None,
    ) -> Tuple[List[EntityRecord], List[ReviewItem]]:
        ...
        for case in sorted(
            cases,
            key=lambda item: (_normalize_name(item.entity_name), item.source_url),
        ):
            if not case.entity_name:
                reviews.append(
                    ReviewItem(
                        review_id=stable_hash(
                            "review", case.case_id, "missing_entity"
                        ),
                        ...
                        details=(
                            "The classifier found a candidate case but no "
                            "reliable entity name could be extracted."
                        ),
            ...
            if score >= 0.93:
                ...
                reviews.append(
                    ReviewItem(
                        review_id=stable_hash(
                            "review", case.case_id, entity.entity_id
                        ),
                        ...
                        case_id=case.case_id,
                        details=(
                            f"Possible duplicate of {entity.entity_name} "
                            f"with similarity {score:.2f}."
                        ),
                        suggested_code=case.final_code,
                    )
                )
        ...
        for entity in entities:
            aliases = [_normalize_name(entity.entity_name)] + [
                _normalize_name(alias) for alias in entity.aliases
            ]
            entity.seed_overlap = any(
                alias in normalized_seed_names for alias in aliases if alias
            )
        return entities, reviews

    def _find_match(
        self, case: CandidateCase, entities: List[EntityRecord]
    ):
        best = None
        best_score = 0.0
        ...
        for entity in entities:
            ...
            if (
                case.final_code
                and entity.canonical_code
                and case.final_code != entity.canonical_code
            ):
                score -= 0.10
            if score > best_score:
                best = entity
    ...
    def _to_entity(self, case: CandidateCase) -> EntityRecord:
        return EntityRecord(
            ...
            entity_name=case.entity_name,
            aliases=sorted(
                set(case.aliases + split_aliases(case.entity_name))
            ),
    ...
    def _merge(
        self, entity: EntityRecord, case: CandidateCase, score: float
    ) -> None:
        entity.aliases = sorted(
            set(
                entity.aliases
                + case.aliases
                + split_aliases(case.entity_name)
            )
        )
