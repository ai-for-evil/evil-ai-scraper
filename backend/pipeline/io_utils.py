"""IO utilities: hashing, normalization, serialization.

Ported from Shiv-Evil-AI-Finder/src/io_utils.py.
"""
...
def stable_hash(*parts: str, length: int = 16) -> str:
    """Deterministic hash from string parts — used for document/entity IDs."""
...
def split_multivalue(value: str) -> List[str]:
    """Split a string on newlines, semicolons, or commas."""
    if not value:
        return []
    bits = re.split(r"[\n;,]+", value)
...
def split_aliases(name: str) -> List[str]:
    """Extract aliases from an entity name (slash-separated, parenthesized)."""
    if not name:
        return []
    aliases = []
...
def write_csv(path: Path, rows: Sequence[Dict[str, Any]]) -> None:
    ...
    with path.open("w", encoding="utf-8", newline="") as handle:
        ...
        for row in rows:
            writer.writerow(
                {key: _flatten_csv_value(row.get(key)) for key in fieldnames}
            )
