"""Taxonomy parser: extract classification taxonomy from PDF and seed CSV.

Ported from Shiv-Evil-AI-Finder/src/taxonomy.py, adapted for our project paths.
"""
...
from backend.pipeline.io_utils import normalize_whitespace, split_aliases, split_multivalue
from backend.schemas import SeedExample, TaxonomyNode
...
# ---------------------------------------------------------------------------
# PDF text extraction (multi-fallback)
# ---------------------------------------------------------------------------

def extract_pdf_text(pdf_path: Path) -> str:
    """Extract text from a PDF using pypdf → PyPDF2 → macOS swift/PDFKit."""
    ...
    raise RuntimeError(
        "Unable to extract PDF text. Install pypdf or run on macOS with swift/PDFKit."
    )
...
def parse_taxonomy_text(text: str) -> List[TaxonomyNode]:
    ...
    def flush() -> None:
        ...
        criteria: List[str] = []
        gray_area = (
            "gray area" in current_title.lower()
            or "criteria currently undecided" in current_title.lower()
        )
        notes: List[str] = []
        if gray_area:
...
def taxonomy_by_code(nodes: List[TaxonomyNode]) -> Dict[str, TaxonomyNode]:
    """Map taxonomy code → TaxonomyNode."""
    return {node.code: node for node in nodes}


# ---------------------------------------------------------------------------
# Seed CSV parsing (column-transposed sheet)
# ---------------------------------------------------------------------------

def load_seed_examples(
    csv_path: Path, nodes: List[TaxonomyNode]
) -> List[SeedExample]:
    """Parse the column-oriented seed CSV into SeedExample records."""
    with csv_path.open(newline="", encoding="utf-8-sig") as handle:
        rows = list(csv.reader(handle))

    taxonomy_map = taxonomy_by_code(nodes)

    # Row indices (0-indexed) for each field in the transposed sheet
    label_row = rows[4]
    reviewer_row = rows[5]
    frame_row = rows[6]
    name_row = rows[7]
    ...
    for column in range(2, max_cols):
        ...
        examples.append(
            SeedExample(
                ...
                subgroup_name=(
                    taxonomy.subgroup_name
                    if taxonomy
                    else (
                        "Not included"
                        if final_code == "Not included"
                        else ""
                    )
                ),
...
# ---------------------------------------------------------------------------
# Lexicon and reference text builders
# ---------------------------------------------------------------------------

def build_code_lexicons(
    nodes: List[TaxonomyNode], seeds: List[SeedExample]
) -> Dict[str, List[str]]:
    """Build per-code keyword lexicons from taxonomy definitions + seeds."""
    lexicons: Dict[str, List[str]] = defaultdict(list)
    for node in nodes:
        lexicons[node.code].extend(
            [node.subgroup_name, node.major_group, node.definition, *node.criteria]
        )
...
def build_reference_texts(
    ...
    """Build per-code reference text lists for prototype similarity."""
