"""Evidence extractor — extract entity names and evidence from document chunks.

Ported from Shiv-Evil-AI-Finder/src/extractor.py.
"""
...
from backend.pipeline.io_utils import split_aliases, stable_hash
from backend.schemas import CandidateCase, ClassificationResult, DocumentChunk, SeedExample


class EvidenceExtractor:
    """Extract entity names and supporting evidence from classified chunks."""

    def __init__(self, seeds: List[SeedExample]) -> None:
        self.alias_index: Dict[str, str] = {}
        self.generic_aliases = {
            "ai", "the ai", "generative ai", "artificial intelligence",
            "platform", "platforms", "services", "service",
            "tool", "tools", "system", "systems", "assistant",
        }
        self.blocked_names = {
            "", "news", "the news", "money matters", "table of contents",
            "cybercrime and abuse ai", "surveillance vendor",
            "nonbank financial company", "official website",
            "u.s. department of the treasury", "department of the treasury",
            "treasury", "misp galaxy", "misp", "xbiz", "wired", "cnbc",
            "vice", "fbi", "interpol", "according", "after", "based",
            "founded", "its", "the", "israeli", "ai-driven",
        }
        self.blocked_prefixes = (
            "both ", "in the prc ", "reported by ",
            "tool website url ", "developer or company ",
        )
        self.headline_terms = {
            "sues", "suing", "identifies", "reported", "reporting",
            "lawsuit", "complaint", "release", "article", "report",
            "attorney", "government", "department", "city", "office",
        }
    ...
    def extract_many(
        ...
        """Extract candidate cases from a classified chunk."""
        raw_names: List[str] = []
        primary_name = self._entity_name(chunk)
        if primary_name:
            raw_names.append(primary_name)
        raw_names.extend(
            self._additional_entities(chunk, classification.final_code)
        )
        ...
        for raw_name in raw_names:
            ...
            if (
                classification.final_code != "Not included"
                and not self._supports_code(evidence_text, classification.final_code)
            ):
                continue
            ...
            cases.append(
                self._build_case(
                    entity_name, evidence_text, confidence,
        ...
        return [
            self._build_case(
                "", chunk.text, classification.confidence,
                chunk, relevance, classification,
            )
        ]
    ...
    def _build_case(
        ...
        suspected_function = (
            ...
            if classification.final_code != "Not included"
            else "uncertain"
        )
        return CandidateCase(
            case_id=stable_hash(
                chunk.chunk_id, entity_name, classification.final_code
            ),
            ...
            relevance_reasons=list(relevance.get("reasons", [])),
            classification_debug={
                "signals": [
                    signal.to_dict()
                    for signal in classification.debug_signals
                ],
                ...
                "evidence_support": self._supports_code(
                    evidence_text, classification.final_code
                ),
            },
    ...
    def _title_candidate(self, title: str) -> str:
        ...
        if any(
            term in lowered for term in self.headline_terms
        ) and not self._looks_like_company(candidate):
            return ""
    ...
    def _looks_like_entity(
        self, value: str, chunk: DocumentChunk | None = None
    ) -> bool:
        ...
        if any(
            token in value
            for token in [
                "GPT", "Bot", "Proxy", "Vision", "Claw", "Locker",
                "Stealer", "Spotter", "PredPol", "Clearview",
                "Nudify", "Surakshini",
            ]
        ):
            return True
        ...
        return bool(
            re.fullmatch(
                r"[A-Z][A-Za-z0-9.-]{2,}(?:\s+[A-Z][A-Za-z0-9.&-]{2,}){0,4}",
                value,
            )
        )

    def _additional_entities(
        self, chunk: DocumentChunk, final_code: str
    ) -> List[str]:
        ...
        if final_code in {"1B", "2A", "2C", "3A", "3B", "3C"}:
            for match in re.findall(
                r"\b([A-Za-z0-9-]{3,}\.(?:ai|app|cc|com|io|love|online|art|net))\b",
                text,
            ):
    ...
    def _normalize_candidate(self, value: str) -> str:
        ...
        if ". " in value:
            ...
            if head and len(head.split()) <= 5:
                ...
                descriptive_tail = tail.split(" ", 1)[0].lower() in {
                    "the", "a", "an", "israeli", "founded", "based"
                }
    ...
    def _clean_candidate_name(
        self, value: str, chunk: DocumentChunk
    ) -> str:
