"""ML building blocks: PrototypeClassifier, KeywordScorer, OptionalSklearnTextClassifier.

Ported from Shiv-Evil-AI-Finder/src/models.py.
Used by the relevance scorer and hybrid classifier.
"""
...
class PrototypeClassifier:
    """Cosine-similarity classifier against prototype vectors per taxonomy code."""
...
class KeywordScorer:
    """Phrase- and token-level keyword scoring against taxonomy lexicons."""

    def __init__(self, lexicons: Dict[str, Sequence[str]]) -> None:
        self.lexicons = {
            code: [entry.lower() for entry in values if entry]
            for code, values in lexicons.items()
        }
        self.lexicon_tokens = {
            code: {
                token
        ...
        for code, lexicon in self.lexicons.items():
            ...
            for phrase in lexicon:
                if len(phrase) >= 4 and phrase in lowered:
                    hits += 1
            token_hits = len(
                query_tokens & self.lexicon_tokens.get(code, set())
            )
            denominator = max(
                3, min(12, len(self.lexicon_tokens.get(code, set())) or 3)
            )
            phrase_score = min(
                1.0, hits / max(1, min(6, max(1, len(lexicon) // 6)))
            )
...
class OptionalSklearnTextClassifier:
    """TF-IDF + Logistic Regression classifier — gracefully disabled if sklearn is missing."""
    ...
    def score(self, text: str) -> Dict[str, float]:
        ...
        return {
            ...
            for label, probability in zip(
                self.pipeline.classes_, probabilities
            )
        }
